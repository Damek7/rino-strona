(function (root, factory) {
    ...
    function runIndexedDb(mode, createRequest) {
      return new Promise((resolve, reject) => {
        ...
        openRequest.addEventListener('success', () => {
          ...
          let requestResult
          request.addEventListener('success', () => { requestResult = request.result }, { once: true })
  ...
  function createSupabaseStore(client, options = {}) {
    ...
    const publicTrainerDto = row => ({
      ...
      level: row.level || 'Każdy poziom',
      experience: row.experience || '',
      specialties: row.specialties || [],
      gallery: (row.gallery || []).sort((a, b) => a.sort_order - b.sort_order).map(item => ({
        id: item.id, url: item.url, alt: item.alt_text, order: item.sort_order, isCover: item.is_cover,
      })),
    })
    ...
    return {
      mode: 'supabase', signUp, signIn, signOut, getSession, getDashboard, listTrainers, getPublicTrainer, listTrainerReviews,
