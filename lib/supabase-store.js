(function (root, factory) {
  ...
  function createSupabaseStore(client) {
    ...
    const pendingStorage = globalThis.sessionStorage
    const fail = error => {
    ...
    function savePendingAvatar(email, avatarDataUrl) {
      try { pendingStorage?.setItem(pendingAvatarKey, JSON.stringify({ email, avatarDataUrl })) } catch {}
    }

    async function completePendingAvatar(user) {
      let pending
      try { pending = JSON.parse(pendingStorage?.getItem(pendingAvatarKey) || 'null') } catch {}
      ...
      try { pendingStorage?.removeItem(pendingAvatarKey) } catch {}
      return avatarUrl
    }
    ...
    async function signUp(input) {
      ...
      if (input.role === 'trainer') {
        if (data.session) user.avatarUrl = await uploadTrainerAvatar(data.user.id, input.avatarDataUrl)
        else savePendingAvatar(email, input.avatarDataUrl)
      }
