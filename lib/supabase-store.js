(function (root, factory) {
  ...
  function createSupabaseStore(client) {
    ...
    const bookingSelect = 'id,client_id,trainer_id,slot_id,starts_at,ends_at,status,payment_status,price,platform_fee,location,created_at,trainer:profiles!bookings_trainer_id_fkey(full_name),client:profiles!bookings_client_id_fkey(full_name)'

    async function currentUser() {
      const { data, error } = await client.auth.getUser()
      fail(error)
      if (!data.user) throw new Error('Zaloguj się, aby kontynuować.')
      const profileResult = await client.from('profiles').select('id,full_name,email,role,avatar_url').eq('id', data.user.id).single()
    ...
    async function signUp(input) {
      const { data, error } = await client.auth.signUp({
        ...
        options: { data: { full_name: String(input.fullName || '').trim(), role: input.role, accepted_terms_at: new Date().toISOString() } },
      })
      fail(error)
    ...
    async function getSession() {
      ...
      const result = await client.from('profiles').select('id,full_name,email,role,avatar_url').eq('id', data.user.id).maybeSingle()
    ...
    async function listConversations() {
      ...
      for (const row of membership.data || []) {
        const members = await client.from('conversation_members').select('user_id,profile:profiles!conversation_members_user_id_fkey(id,full_name,email,role)').eq('conversation_id', row.conversation_id)
        fail(members.error)
        const messages = await client.from('messages').select('id,conversation_id,sender_id,body,created_at').eq('conversation_id', row.conversation_id).order('created_at', { ascending: false }).limit(1)
        fail(messages.error)
        const other = members.data?.find(item => item.user_id !== user.id)?.profile
        items.push({ id: row.conversation.id, bookingId: row.conversation.booking_id, createdAt: row.conversation.created_at, other: other ? userDto(null, other) : null, lastMessage: messages.data?.[0] || null })
      }
    ...
    return {
      ...
      listMessages, sendMessage, setAvailability, getEarnings, getPreferences, savePreferences,
      saveTrainerProfile, listClients, createReview, subscribeToMessages,
    }
  }
